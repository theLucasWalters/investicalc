...
def percentChange(get1, get2):
    ...
    test = Label(pc, text=f'Answer = {rndANS}%').pack()
...
def percentChange_window():
    ...
    wrn = Label(pc, text="Make sure to remove all commas before pressing 'submit'").pack()
    num1 = Entry(pc, width=20, borderwidth=2.5)
    num2 = Entry(pc, width=20, borderwidth=2.5)
    num1.pack(); num2.pack()
    calculate = Button(pc, text='Submit', command=lambda: percentChange(get1=num1, get2=num2))
    calculate.pack()
